...
int sz[maxn*2],dep[maxn*2],rt_tag;
pair<int,int> rt;
// rt连边是定向的，{子节点, 父节点}
int dfs_rt(int u,int fa,int e_tag,int tot,int dep){
    // cout<<"  dfs: "<<u<<" "<<fa<<" "<<tot<<" "<<dep<<endl;
    
    // 这里的v, tag在下面需要用到，所以顺序很重要!!!
    sz[u]=1;int maxn=0,max_dep=dep,v,tag;
    for(auto &it:e[u]){
        
        if(it.first==fa||e_vis[abs(it.second)]) continue;
        v=it.first,tag=it.second;
        max_dep=max(max_dep,dfs_rt(v,u,tag,tot,dep+(tag>0)));
        sz[u]+=sz[v];
        maxn=max(maxn,sz[v]);
    }
...
void dfs_div(int u,int fa,int tot=u_cnt){
    ...
    for(int i=0;i<len;i++) a[i].y=b[i].x;
    work(a,len);

    for(int i=0;i<len;i++){
        dis_cnt[i+(rt_tag==-1?0:1)]+=(ll)(a[i].y + 0.5)/2;
    }
